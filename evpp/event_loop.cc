...
void EventLoop::Init() {
    ...
    /*
     * 替换pending_functors_的实现方法
     * folly::MPMCQueue
     * edit by chijinxin
     * 2018/05/27
     */
    const size_t kPendingFunctorCount = 1024 * 32;
    this->pending_functors_ = new folly::MPMCQueue<Functor*>(kPendingFunctorCount);
...
void EventLoop::QueueInLoop(const Functor& cb) {
    DLOG_TRACE << "pending_functor_count_=" << pending_functor_count_ << " PendingQueueSize=" << GetPendingQueueSize() << " notified_=" << notified_.load();
    {
        ...
        while(!pending_functors_->writeIfNotFull(f)) { }
...
void EventLoop::DoPendingFunctors() {
    ...
    Functor* f = nullptr;
    while(pending_functors_->readIfNotEmpty(f))
    {
        (*f)();
        delete f;
        --pending_functor_count_;
...
size_t EventLoop::GetPendingQueueSize() {
    /*
     * 替换为folly::MPMCQueue实现pending_functors_
     * edit by chijinxin
     * 2018/05/27
     */
...
bool EventLoop::IsPendingQueueEmpty() {
     ...
     * 2018/05/27
     */
    return pending_functors_->isEmpty();
//#ifdef H_HAVE_BOOST
